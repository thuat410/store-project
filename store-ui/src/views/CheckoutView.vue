...
const steps = computed(() => [
  { id: 1, name: t('checkout.steps.shipping'), icon: '📦' },
  { id: 2, name: t('checkout.steps.payment'), icon: '💳' },
  { id: 3, name: t('checkout.steps.review'), icon: '✅' }
])

const paymentMethods = computed(() => [
  { value: 'card', label: t('checkout.creditDebitCard'), icon: '💳' },
  { value: 'paypal', label: t('checkout.paypal'), icon: '🅿️' },
  { value: 'cash', label: t('checkout.cashOnDelivery'), icon: '💵' }
])

function validateShipping() {
  const errors = {}
  if (!shippingForm.value.firstName) errors.firstName = t('checkout.required')
  if (!shippingForm.value.lastName) errors.lastName = t('checkout.required')
  if (!shippingForm.value.email) errors.email = t('checkout.required')
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(shippingForm.value.email)) errors.email = t('checkout.invalidEmail')
  if (!shippingForm.value.phone) errors.phone = t('checkout.required')
  if (!shippingForm.value.address) errors.address = t('checkout.required')
  if (!shippingForm.value.city) errors.city = t('checkout.required')
  if (!shippingForm.value.zipCode) errors.zipCode = t('checkout.required')
  shippingErrors.value = errors
  return Object.keys(errors).length === 0
}

function nextStep() {
  if (currentStep.value === 1 && !validateShipping()) return
...
async function placeOrder() {
  ...
  try {
    const orderData = {
      items: items.value.map(item => ({ productId: item.id, quantity: item.quantity, price: item.price })),
    ...
    clearCart()
    router.push({ name: 'OrderSuccess', query: { orderId: order?.id || 'ORD-' + Date.now() } })
  } catch (err) {
...
function paymentLabel(value) {
  const map = { card: t('checkout.creditDebitCard'), paypal: t('checkout.paypal'), cash: t('checkout.cashOnDelivery') }
    ...
    <div class="bg-white border-b border-gray-100">
      <div class="container-custom py-3">
        <nav class="flex items-center gap-2 text-sm text-gray-500">
          ...
          <RouterLink to="/cart" class="hover:text-primary-600">{{ t('cart.title') }}</RouterLink>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
      ...
      <div class="flex items-center justify-center mb-8">
        <div class="flex items-center">
          <template v-for="(step, index) in steps" :key="step.id">
            <div class="flex items-center">
              <div :class="['w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300', currentStep === step.id ? 'bg-primary-600 text-white ring-4 ring-primary-100' : currentStep > step.id ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-500']">
                <svg v-if="currentStep > step.id" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                <span v-else>{{ step.id }}</span>
              </div>
              <span :class="['ml-2 text-sm font-medium hidden sm:block', currentStep >= step.id ? 'text-gray-900' : 'text-gray-400']">{{ step.name }}</span>
            </div>
            <div v-if="index < steps.length - 1" :class="['w-16 md:w-24 h-1 mx-3 rounded-full transition-colors duration-300', currentStep > step.id ? 'bg-primary-600' : 'bg-gray-200']"></div>
          </template>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
          ...
          <div v-if="currentStep === 1" class="bg-white rounded-2xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t('checkout.shippingAddress') }}</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label class="form-label">{{ t('checkout.emailAddress') }}</label>
              ...
              <div class="sm:col-span-2">
                ...
                <input v-model="shippingForm.address" type="text" class="input-field" :class="{ 'border-red-400': shippingErrors.address }" placeholder="123 Main Street" />
              ...
              <div>
                <label class="form-label">{{ t('checkout.state') }}</label>
              ...
              <div>
                <label class="form-label">{{ t('checkout.country') }}</label>
              ...
              <div class="sm:col-span-2">
                <label class="form-label">{{ t('checkout.deliveryNotes') }}</label>
          ...
          <div v-else-if="currentStep === 2" class="bg-white rounded-2xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t('checkout.paymentMethod') }}</h2>
            <div class="space-y-3 mb-6">
              <label v-for="method in paymentMethods" :key="method.value" :class="['flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors duration-200', paymentMethod === method.value ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300']">
                <input v-model="paymentMethod" type="radio" :value="method.value" class="accent-primary-600" />
            ...
            <div v-if="paymentMethod === 'card'" class="space-y-4">
              ...
              <div>
                <label class="form-label">{{ t('checkout.cardholderName') }}</label>
            ...
            <div v-else-if="paymentMethod === 'paypal'" class="bg-blue-50 rounded-xl p-4 text-center">
              <p class="text-blue-600 font-medium">{{ t('checkout.paypalRedirect') }}</p>
            </div>
            <div v-else class="bg-gray-50 rounded-xl p-4">
              <p class="text-gray-600 font-medium">{{ t('checkout.cashDesc') }}</p>
            </div>
          </div>

          <!-- Step 3: Review -->
          <div v-else class="bg-white rounded-2xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t('checkout.reviewYourOrder') }}</h2>

            <div class="border border-gray-100 rounded-xl p-4 mb-4">
              <div class="flex items-center justify-between mb-2">
                <h3 class="font-semibold text-gray-900">{{ t('checkout.shippingAddress') }}</h3>
                <button @click="currentStep = 1" class="text-xs text-primary-600 hover:underline">{{ t('checkout.edit') }}</button>
              </div>
            ...
            <div class="border border-gray-100 rounded-xl p-4 mb-4">
              <div class="flex items-center justify-between mb-2">
                <h3 class="font-semibold text-gray-900">{{ t('checkout.paymentMethod') }}</h3>
                <button @click="currentStep = 2" class="text-xs text-primary-600 hover:underline">{{ t('checkout.edit') }}</button>
              </div>
              <p class="text-sm text-gray-600">{{ paymentLabel(paymentMethod) }}</p>
            </div>

            <div class="border border-gray-100 rounded-xl p-4">
              <h3 class="font-semibold text-gray-900 mb-3">{{ t('checkout.yourItems') }} ({{ items.length }})</h3>
              <div class="space-y-3">
                <div v-for="item in items" :key="item.id" class="flex items-center gap-3">
                  <div class="w-12 h-12 bg-gray-50 rounded-lg overflow-hidden flex-shrink-0 border border-gray-100">
                    <img :src="item.image" :alt="item.nameKey ? t(item.nameKey) : item.name" class="w-full h-full object-cover" />
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900 line-clamp-1">{{ item.nameKey ? t(item.nameKey) : item.name }}</p>
            ...
            <div v-if="submitError" class="mt-4 bg-red-50 text-red-600 rounded-xl p-3 text-sm">{{ submitError }}</div>
          </div>

          <!-- Navigation buttons -->
          <div class="flex items-center justify-between mt-4">
            <button v-if="currentStep > 1" @click="prevStep" class="btn-secondary px-6 py-3 rounded-xl flex items-center gap-2">
            ...
            <button v-if="currentStep < 3" @click="nextStep" class="btn-primary px-8 py-3 rounded-xl flex items-center gap-2">
              {{ t('checkout.continue') }}
            ...
            <button v-else @click="placeOrder" :disabled="isSubmitting" class="btn-primary px-8 py-3 rounded-xl flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-2xl shadow-sm p-5 sticky top-24">
            <h2 class="font-bold text-gray-900 text-lg mb-4">{{ t('checkout.orderSummary') }}</h2>
            <div class="space-y-3 text-sm">
              ...
              <div class="border-t border-gray-100 pt-3 flex justify-between">
                <span class="font-bold text-gray-900 text-base">{{ t('checkout.total') }}</span>
                <span class="text-xl font-extrabold text-primary-600">${{ orderTotal }}</span>
              </div>
            </div>

            <div class="mt-4 border-t border-gray-100 pt-4">
              <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{{ t('checkout.yourItems') }}</p>
              <div class="space-y-2 max-h-48 overflow-y-auto">
                <div v-for="item in items" :key="item.id" class="flex items-center gap-2 text-xs">
                  ...
                  <span class="flex-1 text-gray-600 line-clamp-1">{{ item.nameKey ? t(item.nameKey) : item.name }} x{{ item.quantity }}</span>
