...
<template>
  <div
    class="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-primary-100 transition-all duration-300 group flex flex-col"
  >
    <!-- Product image -->
    <div class="relative overflow-hidden bg-gray-50" :class="compact ? 'h-36' : 'h-48'">
      ...
      <button
        ...
        class="absolute top-2 right-2 w-8 h-8 bg-white rounded-full shadow-sm flex items-center justify-center transition-all duration-200 opacity-0 group-hover:opacity-100 hover:scale-110"
      ...
      <div class="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
        <button
          ...
          :disabled="isAdding"
          class="w-full bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium py-2.5 transition-colors duration-200 flex items-center justify-center gap-2"
        >
          <svg v-if="!isAdding" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="p-3 flex flex-col flex-1">
      ...
      <RouterLink
        ...
        class="text-sm font-semibold text-gray-900 hover:text-primary-600 transition-colors duration-200 line-clamp-2 leading-snug mb-1.5"
      >
        {{ product.name }}
      </RouterLink>
      ...
      <div class="flex items-center justify-between mt-auto">
        ...
        <button
          @click="handleAddToCart"
          :disabled="isAdding"
          class="w-8 h-8 rounded-full bg-primary-600 hover:bg-primary-700 text-white flex items-center justify-center transition-all duration-200 hover:scale-110 flex-shrink-0"
